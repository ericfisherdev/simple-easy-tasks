...
func (do *dbOptimizer) RunVacuum(ctx context.Context, tables []string) error {
	if len(tables) == 0 {
		...
		slog.Info("Running VACUUM on entire database")
		if _, err := do.db.ExecContext(ctx, "VACUUM;"); err != nil {
			return fmt.Errorf("failed to vacuum database: %w", err)
		}
	} else {
		// Vacuum specific tables
		for _, table := range tables {
			slog.Info("Running VACUUM on table", "table", table)
			// SQLite doesn't support VACUUM on individual tables
			// But we can run REINDEX for specific tables
			query := fmt.Sprintf("REINDEX %s;", table)
			if _, err := do.db.ExecContext(ctx, query); err != nil {
				slog.Warn("Failed to reindex table", "table", table, "error", err)
			}
		}
	}
...
func (do *dbOptimizer) UpdateTableStatistics(ctx context.Context, tables []string) error {
	...
	if len(tables) == 0 {
		slog.Info("Running ANALYZE on entire database")
		if _, err := do.db.ExecContext(ctx, "ANALYZE;"); err != nil {
			return fmt.Errorf("failed to analyze database: %w", err)
		}
	} else {
		for _, table := range tables {
			slog.Info("Running ANALYZE on table", "table", table)
			query := fmt.Sprintf("ANALYZE %s;", table)
			if _, err := do.db.ExecContext(ctx, query); err != nil {
				slog.Warn("Failed to analyze table", "table", table, "error", err)
			}
		}
	}
